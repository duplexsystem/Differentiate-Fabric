...
public class BlockHelper extends AbstractHelper {
	...
	public BlockHelper(Registrator parent) {
		super(parent);
		itemRegister = this.parent.getHelper(Registry.ITEM);
	}
	...
	public Block createFuelBlock(String name, Block block, Item.Settings props, int burnTime) {
		Block registeredBlock = Registry.register(Registry.BLOCK, this.parent.id(name), block);
		Item registeredItem = this.itemRegister.createItem(name, new BlockItem(registeredBlock, props));
		ItemHelper.FUEL.put(registeredItem, burnTime);
		return registeredBlock;
	}

	public Block createSimpleFuelBlock(String name, Block block, ItemGroup group, int burnTime, String ...mods) {
